...
public class Persistence {
    ...
    public void setIntrinsics(List<String> locations) {
        ...
        List<Exception> invalid = new ArrayList<>();

        try {
            BurnedInArgumentsObjectFactory factory = new BurnedInArgumentsObjectFactory(ObjectAssembly.class, _statements, "-");
            XMLBeanAssembler assembler = new XMLBeanAssembler(factory);
            for (String location: locations) {
                try (java.io.InputStream stream = resolver.getResource(location).getInputStream()) {
                    assembler.build(stream);
                } catch (Exception x) {
                    invalid.add(x);
                }
            }
        } catch (javax.xml.parsers.ParserConfigurationException|org.xml.sax.SAXException x) {
            throw new RuntimeException("Intrinsic assembly failure", x);
        }

        if (invalid.size() > 0) {
            throw new RuntimeException("Failure in loading intrinsics from: " + invalid);
        }
    }
